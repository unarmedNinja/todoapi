...
export class MongoDBClient {
  ...
  public insert(collection: string, model: Todo, result: (error, data) => void): void {
    this.db.collection(collection).insertOne(model, (error, insert) => {
      return result(error, insert.ops[0]);
    });
  }

  public update(collection: string, objectId: string, model: Todo, result: (error, data) => void): void {
